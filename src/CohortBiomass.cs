        ...
        : Landis.Library.UniversalCohorts.ICalculator
    {
        ...
        public double ComputeChange(ICohort cohort, ActiveSite site, out ExpandoObject otherParams)
        {
            ...
            if (Main.Month == 6)  //July = 6
            {
                ...
                if (defoliation > 0.0)
                {
                    ...
                    //PlugIn.ModelCore.UI.WriteLine("Defoliation.Month={0:0.0}, LeafBiomass={1:0.00}, DefoliatedLeafBiomass={2:0.00}, TotalLeafMort={2:0.00}", Main.Month, cohort.Data.AdditionalParameters.LeafBiomass, defoliatedLeafBiomass , mortalityAge[1]);
            ...
            //if((totalMortality[1] + defoliatedLeafBiomass) > cohort.Data.AdditionalParameters.LeafBiomass)
            //   PlugIn.ModelCore.UI.WriteLine("Warning: Leaf Mortality exceeds cohort leaf biomass. M={0:0.0}, B={1:0.0}, DefoLeafBiomass={2:0.0}, defoliationIndex={3:0.0}", totalMortality[1], cohort.Data.AdditionalParameters.LeafBiomass, defoliatedLeafBiomass, defoliation);
                                         ...
                                         double[] mortalityAge)
        {
            ...
            if (Double.IsNaN(limitT) || Double.IsNaN(limitH20) || Double.IsNaN(limitLAI) || Double.IsNaN(competition_limit) || Double.IsNaN(limitN))
            {
                ...
                PlugIn.ModelCore.UI.WriteLine("  Yr={0},Mo={1}.     Other Information: MaxB={2}, Bsite={3}, Bcohort={4:0.0}, SoilT={5:0.0}.", PlugIn.ModelCore.CurrentTime, Main.Month + 1, SpeciesData.Max_Biomass[cohort.Species], (int)siteBiomass, (cohort.Data.AdditionalParameters.WoodBiomass + cohort.Data.AdditionalParameters.LeafBiomass), SiteVars.SoilTemperature[site]);
            ...
            if (PlugIn.ModelCore.CurrentTime > 0 && OtherData.CalibrateMode)
            {
                ...
                CalibrateLog.cohortB = (cohort.Data.AdditionalParameters.WoodBiomass + cohort.Data.AdditionalParameters.LeafBiomass);
        ...
        private double[] ComputeAgeMortality(ICohort cohort, ActiveSite site)
        {
            ...
            double totalBiomass = (double)(cohort.Data.AdditionalParameters.WoodBiomass + cohort.Data.AdditionalParameters.LeafBiomass);
            ...
            double M_AGE_wood = cohort.Data.AdditionalParameters.WoodBiomass * monthAdjust *
                                    Math.Exp((double)cohort.Data.Age / max_age * d) / Math.Exp(d);

            double M_AGE_leaf = cohort.Data.AdditionalParameters.LeafBiomass * monthAdjust *
                                    Math.Exp((double)cohort.Data.Age / max_age * d) / Math.Exp(d);
        ...
        private double[] ComputeGrowthMortality(ICohort cohort, ActiveSite site, double siteBiomass, double[] AGNPP)
        {
            ...
            double M_wood_fixed = cohort.Data.AdditionalParameters.WoodBiomass * FunctionalType.Table[SpeciesData.FuncType[cohort.Species]].MonthlyWoodMortality;
            double M_leaf = 0.0;
            ...
            {
                M_leaf = cohort.Data.AdditionalParameters.LeafBiomass / (double)SpeciesData.LeafLongevity[cohort.Species] / 12.0;  //Needle deposit spread across the year.
            ...
            else
            {
                ...
                {
                    M_leaf = cohort.Data.AdditionalParameters.LeafBiomass / 2.0;  //spread across 2 months

                }
                if (Main.Month + 2 > FunctionalType.Table[SpeciesData.FuncType[cohort.Species]].FoliageDropMonth)
                {
                    M_leaf = cohort.Data.AdditionalParameters.LeafBiomass;  //drop the remainder
                }
            }
            ...
            if (SiteVars.Cohorts[site] != null)
            {
                foreach (ISpeciesCohorts speciesCohorts in SiteVars.Cohorts[site])
                {
                    foreach (ICohort cohort in speciesCohorts)
                    {
                        if (SpeciesData.Grass[cohort.Species])
                        {
                            grassTotal += cohort.Data.AdditionalParameters.WoodBiomass;
                        }
                    }
                }
            }
